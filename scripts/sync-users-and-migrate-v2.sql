-- =====================================================
-- 同步用户并迁移单词到自定义单词书 V2
-- 使用占位符密码（用户通过 auth.users 认证，这里只是满足外键约束）
-- =====================================================

-- 第 1 步：将 auth.users 中的用户同步到 vocab_app.users
INSERT INTO vocab_app.users (id, email, password_hash, created_at, updated_at)
SELECT 
    au.id,
    au.email,
    'MIGRATED_FROM_AUTH',  -- 占位符，实际认证使用 auth.users
...
WHERE wb.source_type = 'custom'
GROUP BY wb.id, wb.name, wb.user_id, wb.word_count;
