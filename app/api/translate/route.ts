...
export async function POST(request: NextRequest) {
  try {
    ...
    const str1 = BAIDU_APP_ID + text + salt + BAIDU_SECRET_KEY;
    const sign = generateMD5(str1);

    console.log('[API Translate] Request:', {
      q: text.substring(0, 50) + (text.length > 50 ? '...' : ''),
      from,
      to,
      appid: BAIDU_APP_ID.substring(0, 5) + '...',
    });
    ...
    console.log('[API Translate] Response:', {
      from: data.from,
      to: data.to,
    ...
    // 合并翻译结果
    const translation = data.trans_result?.map(r => r.dst).join('\n') || '';
